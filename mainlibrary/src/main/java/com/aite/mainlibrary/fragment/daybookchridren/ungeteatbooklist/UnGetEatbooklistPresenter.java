...
public class UnGetEatbooklistPresenter extends BasePresenterImpl<UnGetEatbooklistContract.View> implements UnGetEatbooklistContract.Presenter {
    @Override
    ...
    @Override
    public void getStartEatQrinformation(HttpParams httpParams, int postion) {
        OkGo.<BaseData<QrBookMorningNoonEatBean>>get(AppConstant.EATGETQRBOOKURL)
                .tag(mView.getContext())
                .params(httpParams)
                .execute(new AbsCallback<BaseData<QrBookMorningNoonEatBean>>() {
                    @Override
                    public BaseData<QrBookMorningNoonEatBean> convertResponse(okhttp3.Response response) throws Throwable {
                        LogUtils.d(response.request());
                        JSONObject jsonObject = new JSONObject(response.body().string());
                        BaseData baseData = BeanConvertor.convertBean(jsonObject.toString(), BaseData.class);
                        if (baseData.getDatas().getError() != null) {
                            ...
                            JSONObject object = jsonObject.optJSONObject("datas");
                            QrBookMorningNoonEatBean qrBookMorningNoonEatBean = BeanConvertor.convertBean(object.toString(), QrBookMorningNoonEatBean.class);
                            ((Activity) mView.getContext()).runOnUiThread(()
                                    -> mView.onGetStartEatQrinformationSuccess(qrBookMorningNoonEatBean, postion));
                        }
                    ...
                    @Override
                    public void onStart(Request<BaseData<QrBookMorningNoonEatBean>, ? extends Request> request) {
                        LogUtils.d("onStart");

                    }

                    @Override
                    public void onSuccess(Response<BaseData<QrBookMorningNoonEatBean>> response) {
                        LogUtils.d("onSuccess");
